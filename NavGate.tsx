    ...
    <div className="menu-panel" ref={panelRef}>
      ...
      {email ? (
        <>
          ...
          <button className="menu-item btn-secondary" onClick={signOut}>Logout</button>
        </>
      ) : (
  ...
  return (
    <div className="relative">
      <nav className="hidden sm:flex items-center gap-2 text-sm">
        ...
        {email ? (
          <>
            <Link className="btn-outline !px-3 !py-1" href="/dashboard">Dashboard</Link>
            <Link className="btn-outline !px-3 !py-1" href="/history">History</Link>
            <button className="btn-secondary !px-3 !py-1" onClick={signOut}>Logout</button>
          </>
        ) : (
          <Link className="btn-outline !px-3 !py-1" href="/login">Login</Link>
        )}
      </nav>
