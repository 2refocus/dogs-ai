 ...
 * Always-JSON responses + Node runtime + multipart/JSON + Supabase upload + Replicate polling
 */
...
async function replicateCreate(imageUrl: string, prompt: string) {
  ...
  if (!res.ok) throw new Error(`Replicate create failed (${res.status}): ${await res.text().catch(()=>"")}`);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (ct.startsWith("application/json")) {
      ...
      if (!file) return fail("Missing file upload", 400);
    ...
    if (!id) return fail("Replicate did not return an id", 502);

    const start = Date.now();
    const timeoutMs = 55_000;
    let outputUrl: string | null = null;
    while (Date.now() - start < timeoutMs) {
      const pred = await replicateGet(id);
      const status = pred?.status;

      if (status === "succeeded" || status === "completed") {
        const out = Array.isArray(pred?.output) ? pred.output : [];
